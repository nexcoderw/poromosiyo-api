...
describe('Poromosiyo session authentication (e2e)', () => {
  ...
  const customerEmail = `phase8-customer-${suffix}@example.test`;

  const adminEmail = `phase8-admin-${suffix}@example.test`;

  const customerPassword = 'Poromosiyo-Phase8-Customer-123!';

  const adminPassword = 'Poromosiyo-Phase8-Admin-123!';

  let customerRegistration: AuthenticationResult | null = null;

  let customerRefresh: AuthenticationResult | null = null;

  let customerLogin: AuthenticationResult | null = null;

  let adminLogin: AuthenticationResult | null = null;
  ...
  it('registers a customer', async () => {
    ...
    customerRegistration = parseAuthenticationResult(response.text);
  });

  it('returns the authenticated customer from /auth/me', async () => {
    const auth = requireAuth(customerRegistration);
  ...
  it('rotates the customer refresh token', async () => {
    ...
    customerRefresh = parseAuthenticationResult(response.text);

    expect(customerRefresh.refreshToken).not.toBe(auth.refreshToken);
  });
  ...
  it('invalidates access tokens after refresh-token reuse revokes the session', async () => {
    const auth = requireAuth(customerRefresh);
  ...
  it('creates a fresh customer session through login', async () => {
    ...
    customerLogin = parseAuthenticationResult(response.text);
  });

  it('logs out the current customer session', async () => {
    const auth = requireAuth(customerLogin);
  ...
  it('logs an admin in through the admin namespace', async () => {
    ...
    adminLogin = parseAuthenticationResult(response.text);
  });

  it('returns the authenticated admin from /admin/me', async () => {
    const auth = requireAuth(adminLogin);
...
function parseAuthenticationResult(text: string): AuthenticationResult {
  ...
  if (typeof value !== 'object' || value === null) {
    throw new Error('Expected authentication response object.');
  }
