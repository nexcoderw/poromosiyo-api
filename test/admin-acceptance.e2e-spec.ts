...
describe('Poromosiyo admin acceptance and hardening (e2e)', () => {
  ...
  const password =
    'Poromosiyo-Admin-Acceptance-123!';

  const superadminEmail =
    `m17-superadmin-${suffix}@example.test`;

  const adminEmail =
    `m17-admin-${suffix}@example.test`;

  const createdAdminEmail =
    `m17-created-admin-${suffix}@example.test`;

  const customerEmail =
    `m17-customer-${suffix}@example.test`;
  ...
  it('rejects malformed governed account IDs', async () => {
    ...
    await request(
      ...
      .get(
        '/api/v1/admin/admins/not-a-uuid',
      )
      .set(
        'Authorization',
        `Bearer ${admin.accessToken}`,
  ...
  it('rejects governance pagination above the maximum limit', async () => {
    await request(
      ...
      .get(
        '/api/v1/admin/customers?limit=101',
      )
      .set(
        'Authorization',
        `Bearer ${admin.accessToken}`,
  ...
  it('blocks CUSTOMER and invalidates both access and refresh credentials', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/customers/${customer.user.id}/block`,
      )
      .set(
        'Authorization',
        `Bearer ${admin.accessToken}`,
  ...
  it('unblocks CUSTOMER without restoring old sessions', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/customers/${customer.user.id}/unblock`,
      )
      .set(
        'Authorization',
        `Bearer ${admin.accessToken}`,
  ...
  it('prevents ADMIN from blocking another ADMIN', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/admins/${createdAdminId}/block`,
      )
      .set(
        'Authorization',
        `Bearer ${admin.accessToken}`,
      ...
      .send({
        reason:
          'Ordinary admin must not do this',
      })
      .expect(403);
  });
  ...
  it('allows SUPERADMIN to block ADMIN and invalidates that admin session', async () => {
    if (!createdAdmin) {
      throw new Error(
        'Created admin login is missing.',
      );
    }
    ...
    await request(
      ...
      .send({
        reason:
          'Milestone 17 administrator block',
      })
      .expect(204);
    ...
    await request(
      ...
      .set(
        'Authorization',
        `Bearer ${createdAdmin.accessToken}`,
      )
      .expect(401);
  ...
  it('never allows normal administrator governance to block SUPERADMIN', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/admins/${superadmin.user.id}/block`,
      )
      .set(
        'Authorization',
        `Bearer ${superadmin.accessToken}`,
