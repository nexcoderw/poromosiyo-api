...
describe('Poromosiyo authentication management (e2e)', () => {
  ...
  const customerEmail =
    `session-customer-${suffix}@example.test`;

  const googleOnlyEmail =
    `google-only-${suffix}@gmail.com`;

  const adminEmail =
    `session-admin-${suffix}@example.test`;

  const customerPassword =
    'Poromosiyo-Session-Customer-123!';

  const googleLocalPassword =
    'Poromosiyo-Google-Local-123!';

  const adminPassword =
    'Poromosiyo-Session-Admin-123!';
  ...
  it('revokes another customer session', async () => {
    ...
    await request(
      ...
      .set(
        'Authorization',
        `Bearer ${registrationAuth.accessToken}`,
      )
      .expect(401);
  });
  ...
  it('reports Google as the only authentication method for a Google-only customer', async () => {
    const auth =
      await createGoogleOnlyCustomer();
  ...
  async function createGoogleOnlyCustomer():
    Promise<AuthenticationResult> {
    const user =
      await prisma.user.create({
        data: {
          fullName:
            'Google Only Customer',
          email:
            googleOnlyEmail,
          image:
            null,
  ...
  async function getGoogleOnlySession():
    Promise<AuthenticationResult> {
    if (!googleOnlyAuth) {
      throw new Error(
        'Google-only session has not been created.',
      );
    }
