...
describe('Poromosiyo admin governance (e2e)', () => {
  ...
  const superadminEmail =
    `superadmin-${suffix}@example.test`;

  const adminEmail =
    `admin-${suffix}@example.test`;

  const createdAdminEmail =
    `created-admin-${suffix}@example.test`;

  const customerEmail =
    `customer-${suffix}@example.test`;

  const password =
    'Poromosiyo-Governance-Password-123!';
  ...
  it('allows SUPERADMIN to use normal admin-protected routes', async () => {
    await request(
      ...
      .set(
        'Authorization',
        `Bearer ${superadminAuth.accessToken}`,
      )
      .expect(200);
  });
  ...
  it('lets ADMIN block a customer and immediately invalidates the customer session', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/customers/${customerAuth.user.id}/block`,
      )
      .set(
        'Authorization',
        `Bearer ${adminAuth.accessToken}`,
  ...
  it('shows customer activity history', async () => {
    const response =
      await request(
        ...
        .get(
          `/api/v1/admin/customers/${customerAuth.user.id}/activities`,
        )
        .set(
          'Authorization',
          `Bearer ${adminAuth.accessToken}`,
  ...
  it('lets ADMIN unblock a customer', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/customers/${customerAuth.user.id}/unblock`,
      )
      .set(
        'Authorization',
        `Bearer ${adminAuth.accessToken}`,
  ...
  it('allows ordinary ADMIN to view admins and admin activities', async () => {
    ...
    await request(
      ...
      .get(
        `/api/v1/admin/admins/${createdAdminId}/activities`,
      )
      .set(
        'Authorization',
        `Bearer ${adminAuth.accessToken}`,
  ...
  it('lets SUPERADMIN block another ADMIN', async () => {
    ...
    await request(
      ...
      .set(
        'Authorization',
        `Bearer ${createdLogin.accessToken}`,
      )
      .expect(401);
  });
  ...
  it('lets SUPERADMIN unblock an ADMIN', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/admins/${createdAdminId}/unblock`,
      )
      .set(
        'Authorization',
        `Bearer ${superadminAuth.accessToken}`,
  ...
  it('does not allow the admin-management block endpoint to target SUPERADMIN', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/admins/${superadminAuth.user.id}/block`,
      )
      .set(
        'Authorization',
        `Bearer ${superadminAuth.accessToken}`,
