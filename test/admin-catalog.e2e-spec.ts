...
describe('Poromosiyo admin catalog (e2e)', () => {
  ...
  const adminEmail =
    `catalog-admin-${suffix}@example.test`;

  const customerEmail =
    `catalog-customer-${suffix}@example.test`;

  const adminPassword =
    'Poromosiyo-Catalog-Admin-123!';

  const customerPassword =
    'Poromosiyo-Catalog-Customer-123!';

  const rootSlug =
    `catalog-root-${suffix}`;

  const childSlug =
    `catalog-child-${suffix}`;

  const brandSlug =
    `catalog-brand-${suffix}`;
  ...
  it('rejects a category hierarchy cycle', async () => {
    await request(
      ...
      .patch(
        `/api/v1/admin/categories/${rootCategoryId}`,
      )
      .set(
        'Authorization',
        `Bearer ${adminAuth.accessToken}`,
  ...
  it('rejects a product without a genuine discount', async () => {
    await request(
      ...
      .send({
        ...
        description:
          'This product must not be accepted.',
        originalPrice:
          '10000.00',
        sellingPrice:
          '10000.00',
  ...
  it('lists products using the admin pagination contract', async () => {
    const response =
      await request(
        ...
        .get(
          '/api/v1/admin/products?page=1&limit=20&status=ACTIVE',
        )
        .set(
          'Authorization',
          `Bearer ${adminAuth.accessToken}`,
