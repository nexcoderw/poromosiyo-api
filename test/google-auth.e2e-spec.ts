...
} from '../src/auth/services/google-id-token-verifier.service';
import {
...
} from '../src/auth/types/google-auth.types';
import {
...
describe('Poromosiyo Google authentication (e2e)', () => {
  ...
  const localCustomerEmail =
    `google-local-${suffix}@gmail.com`;

  const newGoogleEmail =
    `google-new-${suffix}@gmail.com`;

  const adminEmail =
    `google-admin-${suffix}@gmail.com`;

  const unsafeEmail =
    `google-third-party-${suffix}@example.net`;

  const customerPassword =
    'Poromosiyo-Google-Customer-123!';

  const adminPassword =
    'Poromosiyo-Google-Admin-123!';
  ...
  async function loginCustomer():
    ...
    return parseAuthenticationResult(
      response.text,
    );
  }
