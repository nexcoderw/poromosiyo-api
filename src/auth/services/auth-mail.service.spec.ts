...
describe('AuthMailService', () => {
  const config = new ConfigService({
    ...
    MAIL_FROM: 'Poromosiyo <no-reply@example.test>',
    CUSTOMER_APP_URL: 'http://localhost:4000',
    ADMIN_APP_URL: 'http://localhost:4001',
  });

  const service = new AuthMailService(config);
  ...
  it('stores password reset tokens only in the test memory transport', async () => {
    ...
    expect(
      service.getLatestMemoryToken('password_reset', 'admin@example.test'),
