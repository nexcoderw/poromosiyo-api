...
export class AuthMailService {
  private readonly mode: 'memory' | 'smtp';
  ...
  private readonly memoryMessages: MemoryMail[] = [];

  constructor(private readonly config: ConfigService) {
    const configuredMode = this.config.getOrThrow<string>('MAIL_DELIVERY_MODE');
    ...
    this.customerAppUrl = this.config.getOrThrow<string>('CUSTOMER_APP_URL');

    this.adminAppUrl = this.config.getOrThrow<string>('ADMIN_APP_URL');
  ...
  async sendEmailVerification(input: {
    ...
    const link = this.createFrontendLink(
      input.role,
      '/verify-email',
      input.token,
    );
    ...
    await this.send({
      ...
      email: input.email,
      token: input.token,
      subject: 'Verify your Poromosiyo email',
      text: [
  ...
  async sendPasswordReset(input: {
    ...
    await this.send({
      ...
      email: input.email,
      token: input.token,
      subject: 'Reset your Poromosiyo password',
      text: [
        `Hello ${input.fullName},`,
        '',
  ...
  async sendPasswordChanged(input: {
    ...
    await this.send({
      ...
      email: input.email,
      token: null,
      subject: 'Your Poromosiyo password was changed',
      text: [
        `Hello ${input.fullName},`,
        '',
  ...
  getLatestMemoryToken(
    ...
    for (let index = this.memoryMessages.length - 1; index >= 0; index -= 1) {
      const message = this.memoryMessages[index];

      if (message?.kind === kind && message.email === email) {
  ...
  private createFrontendLink(
    ...
    const baseUrl = role === 'ADMIN' ? this.adminAppUrl : this.customerAppUrl;
