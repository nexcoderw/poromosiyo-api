...
export class GoogleAuthService {
  ...
  constructor(
    ...
    private readonly verifier: GoogleIdTokenVerifierService,
  ...
  async loginCustomer(
    ...
    const linked = await this.findGoogleAccount(identity.subject);

    if (linked) {
      return this.authenticateLinkedUser(linked.user, 'CUSTOMER', metadata);
    }
    ...
    try {
      return await this.createGoogleCustomer(identity, metadata);
    } catch (error: unknown) {
      if (!isPrismaErrorCode(error, 'P2002')) {
        throw error;
      }

      const accountAfterRace = await this.findGoogleAccount(identity.subject);
  ...
  async loginAdmin(
    ...
    return this.authenticateLinkedUser(linked.user, 'ADMIN', metadata);
  }

  async linkCustomer(principal: AuthPrincipal, idToken: string): Promise<void> {
    await this.linkGoogleAccount(principal, idToken, 'CUSTOMER');
  }

  async linkAdmin(principal: AuthPrincipal, idToken: string): Promise<void> {
    await this.linkGoogleAccount(principal, idToken, 'ADMIN');
  }
  ...
  private async linkGoogleAccount(
    ...
    const accountBySubject = await this.prisma.authAccount.findFirst({
      where: {
        provider: 'GOOGLE',
    ...
    if (accountBySubject) {
      if (accountBySubject.userId === user.id) {
        await this.verifyEmailFromGoogleWhenAuthoritative(user, identity);
    ...
    const existingProvider = await this.prisma.authAccount.findFirst({
      where: {
        userId: user.id,
        provider: 'GOOGLE',
      },
    ...
    try {
      await this.prisma.$transaction(async (transaction) => {
        ...
        if (identity.emailAuthoritative && user.emailVerifiedAt === null) {
      ...
      if (isPrismaErrorCode(error, 'P2002')) {
        throw new ConflictException('This Google account cannot be linked.');
      }
  ...
  private async findGoogleAccount(subject: string) {
...
function toAuthenticatedUser(user: GoogleAuthUser): AuthenticatedUser {
  return {
    id: user.id,
