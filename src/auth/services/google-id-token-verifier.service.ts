...
export class GoogleIdTokenVerifierService {
  ...
  async verify(idToken: string): Promise<VerifiedGoogleIdentity> {
    const enabled = this.config.get<boolean>('GOOGLE_AUTH_ENABLED') ?? false;
    ...
    try {
      ...
      return {
        ...
        image: resolveImage(payload.picture),
      };
    } catch (error: unknown) {
      if (
...
function isAuthoritativeGoogleEmail(
  ...
  return (
    ...
    (typeof hostedDomain === 'string' && hostedDomain.trim().length > 0)
  );
}
...
function resolveFullName(
  ...
  const fallback = normalizeName(localPart.replace(/[._+-]+/g, ' '));

  return (fallback || 'Google User').slice(0, 150);
}
