...
export class AdminAdminsService {
  ...
  async list(
    ...
    const where = {
      role: {
        in: ['ADMIN' as const, 'SUPERADMIN' as const],
      },
  ...
  ): Promise<GovernanceUserResponse> {
      ...
      const user = await this.prisma.$transaction(async (transaction) => {
        ...
        await transaction.userActivity.create({
          data: {
            ...
            action: GOVERNANCE_ACTIVITY_ACTION.ADMIN_CREATED,
            resourceType: 'USER',
  ...
  block(
    ...
    return this.governance.block({
      ...
      reason,
      metadata,
      action: GOVERNANCE_ACTIVITY_ACTION.ADMIN_BLOCKED,
    });
  }
  ...
  unblock(
    ...
    return this.governance.unblock({
      ...
      actor,
      metadata,
      action: GOVERNANCE_ACTIVITY_ACTION.ADMIN_UNBLOCKED,
    });
  }
}
...
function serializeUser(user: {
  ...
  if (user.role !== 'ADMIN' && user.role !== 'SUPERADMIN') {
    throw new Error(`Unsupported admin role: ${user.role}`);
  }
